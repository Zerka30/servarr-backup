...
class Prowlarr(Server):
    def __init__(self, instance_name):
        ...
        instances = config.get("backups", {}).get("starrs", {}).get("prowlarr", [])
        prowlarr_config = next((inst for inst in instances if inst['name'] == instance_name), None)
    ...
    def delete_server_backup(self, backup_id):
        url = f"{self.url}/api/v1/system/backup/{backup_id}"
        headers = {
            "X-Api-Key": self.api_key,
