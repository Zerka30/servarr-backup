...
class Prowlarr(Server):
    def __init__(self):
        ...
        prowlarr_config = config.get("starrs", {}).get("prowlarr", {})
        s3_config = config.get("backup", {}).get("s3", {})
        url = prowlarr_config.get("url")
        api_key = prowlarr_config.get("api_key")

        if not url or not api_key:
            raise ValueError("Prowlarr URL or API Key is missing in the configuration.")
    ...
    def backup(self):
        ...
        if backup_path:
            ...
            upload_success = self.s3_bucket.upload_file(backup_path, f"prowlarr/{s3_key}")
            ...
            if upload_success:
                ...
                # Delete Backup from Prowlarr server
    ...
    def create_backup(self):
        ...
        logger.info("Starting backup creation for Prowlarr.")
        ...
        if res.status_code == 201:
            ...
            if command_id:
                if self.wait_for_completion(command_id):
                    logger.info("Completed backup for Prowlarr successfully.")
                else:
                    logger.error("Failed to complete backup for Prowlarr.")
            else:
                logger.error("Failed to retrieve command ID for backup creation.")
        else:
            logger.error(f"Failed to create backup for Prowlarr. Status code: {res.status_code}")
            logger.debug(f"Response body: {res.text}")


    def delete_backup(self, backup_name):
        logger.info(f"Deleting backup '{backup_name}' from S3 for Prowlarr.")
        try:
            self.s3_bucket.delete_file(f"prowlarr/{backup_name}")
            return True
    ...
    def list_backups(self):
        return self.s3_bucket.list("prowlarr")


    def download_latest_backup(self):
        url = f"{self.url}/api/v1/system/backup"
        headers = {
            "X-Api-Key": self.api_key,
    ...
    def wait_for_completion(self, command_id):
        url = f"{self.url}/api/v1/command/{command_id}"
        headers = {
            "X-Api-Key": self.api_key,
            "Accept": "application/json",
            "Content-Type": "application/json"
